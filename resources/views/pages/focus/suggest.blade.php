    ...
    @if($recommendations->count() > 0)
        <div class="space-y-6">
            @foreach($recommendations as $index => $recommendation)
                <div class="group bg-white dark:bg-[#161b22] border sidebar-border rounded-[10px] p-6 shadow-sm hover:shadow-xl hover:shadow-brand-500/5 transition-all">
                    <div class="flex flex-col md:flex-row gap-6">
                        <div class="shrink-0">
                            <div class="w-12 h-12 bg-brand-50 dark:bg-brand-950/30 text-brand-600 dark:text-brand-400 rounded-[10px] flex items-center justify-center text-xl font-black border border-brand-100 dark:border-brand-900/50">
                                {{ $index + 1 }}
                            </div>
                        </div>
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-start justify-between gap-4">
                                ...
                                <div class="flex items-center gap-1 shrink-0">
                                    <a href="{{ route('tasks.edit', $recommendation['task']) }}" class="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-[10px] text-slate-400 hover:text-brand-600 transition-colors">
                            ...
                            <div class="bg-slate-50 dark:bg-[#0d1117] border sidebar-border rounded-[10px] p-4">
        ...
        <div class="mt-10 pt-6 border-t border-slate-100 dark:border-slate-800 flex justify-center md:justify-start">
            <a href="{{ route('tasks.index') }}" class="flex items-center gap-2 px-6 py-3 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-[10px] font-bold text-sm hover:bg-slate-200 dark:hover:bg-slate-700 transition-all">
