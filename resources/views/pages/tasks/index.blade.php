        ...
        <header class="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <div class="flex items-center gap-3 shrink-0">
                <a href="{{ route('focus.suggest') }}"
                    class="app-btn app-btn-soft flex items-center gap-2 px-4 py-2.5 font-bold text-sm">
                    <i data-lucide="sparkles" class="w-4 h-4"></i>
                    Suggest Task
                </a>
                <a href="{{ route('tasks.create') }}"
                    class="app-btn app-btn-primary flex items-center gap-2 px-5 py-2.5 font-bold text-sm">
            ...
            <div class="flex flex-wrap gap-3 items-center">
                <div class="flex-[2_1_300px] relative group">
                    <i data-lucide="search"
                        class="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-brand-600 transition-colors"></i>
                    <input type="text" name="search" placeholder="Quick search tasks..." value="{{ request('search') }}"
                        onkeyup="searchTasks(event)" class="app-input pl-11 pr-4 py-2.5 text-sm dark:text-white">
                </div>

                <div class="flex-[1_1_160px]">
                    <select name="status" onchange="filterTasks(this)"
                        class="app-input px-4 py-2.5 text-sm dark:text-slate-300">
                ...
                <div class="flex-[1_1_160px]">
                    <select name="tag" onchange="filterTasks(this)"
                        class="app-input px-4 py-2.5 text-sm dark:text-slate-300">
                ...
                <div class="flex-[1_1_160px]">
                    <select name="deadline" onchange="filterTasks(this)"
                        class="app-input px-4 py-2.5 text-sm dark:text-slate-300">
        ...
        <div class="space-y-4">
                ...
                <div class="app-card group p-5 hover:shadow-xl hover:shadow-brand-500/5 transition-all">
                    <div class="flex flex-wrap md:flex-nowrap items-start gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            <h3
                                class="text-lg font-bold truncate transition-all {{ $task->status ? 'line-through text-slate-400' : 'text-slate-800 dark:text-slate-100' }}">
                                {{ $task->title }}
                            </h3>
                            ...
                            <div class="flex flex-wrap items-center gap-y-3 gap-x-6 mt-4">
                                @if ($task->deadline)
                                    <div class="flex items-center gap-2 text-[11px] font-bold uppercase tracking-wider">
                                        <i data-lucide="calendar"
                                            class="w-3.5 h-3.5 {{ $task->isOverdue() ? 'text-red-500' : 'text-slate-400' }}"></i>
                                        <span
                                            class="{{ $task->isOverdue() ? 'text-red-500' : 'text-slate-500 dark:text-slate-400' }}">
                                            {{ $task->deadline->format('M j, Y') }}
                                            @if ($task->isOverdue())
                                                <span
                                                    class="app-badge app-badge-danger ml-1 px-1.5 py-0.5 text-[9px]">Overdue</span>
                                            @endif
                                        </span>
                                    </div>
                                @endif
                                ...
                                <div
                                    class="flex items-center gap-2 text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <div class="flex gap-1">
                                            @foreach ($task->systemTags as $tag)
                                                <span class="text-[10px] px-2 py-0.5 border font-semibold"
                                                    style="background-color: {{ $tag['color'] }}1a; color: {{ $tag['color'] }}; border-color: {{ $tag['color'] }}33; border-radius: 5px;">
                                                    {{ $tag['name'] }}
                                                </span>
                                            @endforeach
                                        </div>
                        ...
                        <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <a href="{{ route('tasks.edit', $task) }}"
                                class="app-btn p-2 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-brand-600 transition-colors"
                            ...
                            <form method="POST" action="{{ route('tasks.destroy', $task) }}" class="inline">
                                @csrf @method('DELETE')
                                <button type="submit"
                                    class="app-btn p-2 hover:bg-red-50 dark:hover:bg-red-900/20 text-slate-400 hover:text-red-600 transition-colors"
                                    style="border-radius: var(--ui-radius);" onclick="return confirm('Delete this task?')">
            ...
            @empty
                <div class="app-panel text-center py-20 border-2 border-dashed">
                    ...
                    <p class="text-slate-500 dark:text-slate-400 mt-2 max-w-xs mx-auto">Clear your filters or start fresh
                        with a new entry.</p>
                    <a href="{{ route('tasks.create') }}"
                        class="app-btn app-btn-primary mt-6 inline-flex items-center gap-2 px-6 py-3 font-bold text-sm hover:scale-105 transition-transform">
